...
function generateSymbol(id, styles) {
  ...
  if (styles.length === 1) {
    ...
    textSize = style.font.size || 16;
    textLineHeight = style.textAttributes.lineHeight || 1.2;
    ...
    styles.forEach(s => {
      ...
      textSize.push(s.style.font.size || 16);
      textLineHeight.push(s.zoom);
      textLineHeight.push(s.style.textAttributes.lineHeight || 1.2);
    });
  }
...
function artboardToStyleLayers(collection) {
  const { artboard, mapLayers } = collection;
  ...
  const layers = artboard.children
    ...
    .reduce((memo, obj) => {
      const { name, zoom } = parseName(obj);

      // Derive type from the `layers` argument. We could get `type` as a
      // property from `parseName(obj)` but a different type may have been
      // used to define something that should be a layer type from the map
      // style.

      let type;
      const matchingLayer = mapLayers.find(d => d.id === name);
      ...
      // create a new entry.
      const exists = memo.findIndex(d => d.name === name);
      if (exists !== -1) {
